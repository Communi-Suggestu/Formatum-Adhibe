...
public class RegionFinder
{
        ...
        RegionOffset end)
    {
        ...
        public boolean isMultiLineBrokenAssignmentStatement() {
        ...
        public boolean hasAssignmentCoveringParameterBlock()
        {
        ...
        public boolean isStart(int lineIndex) {
            return start().lineOffset() == lineIndex;
        }

        public int inRegionLineOffset(int lineIndex) {
            return lineIndex - start().lineOffset();
        }

        public List<Region> parentalRegionChain(final int lineIndex, final int firstCharacterIndex) {
        ...
        public boolean isIsolatedStartOrEndOperand(final int lineIndex) {
    ...
    private record BracedRegionWrapper(int index, boolean isLambdaOrAnonymousClass) {}
    private record StatementStartWrapper(@Nullable Integer start, int codeBlockStart) {}
    private record ParameterRegionStartInfo(Integer index, boolean isParameter) {}
    ...
    public Region findRoot(String content)
    {
        ...
        for (int i = 0; i < content.length(); i++)
        {
            ...
            if (c == '(')
            {
                ...
                parameterizedRegionStack.push(new ParameterRegionStartInfo(i, isParameterBlock));
                if (isParameterBlock) {
                    childrenStack.push(currentChildren);
                    currentChildren = new ArrayList<>();
                    continue;
